...
?>
	<div class="row">
        <div class="col-lg-12">
            <h3>User Accounts | <small>Simple User Management</small>&nbsp;&nbsp;<button data-toggle="modal" data-target="#newUserModal" class="btn btn-raised btn-primary">Add User</button></small></h3>
    			...
    			<tbody>
    				...
    				if(count($usersAr) -1 != 0){
    					foreach($usersAr as $user){
    						...
    						if($user_ex[0] != ""){
    							if($user_ex[0] == "root"){
    								echo '<tr><td>'.$user_ex[0].'</td><td><button class="btn btn-raised btn-primary" onClick="changePwd('.$u1.');">Change Password</button></td></tr>';
    							} else {
    								$u1 = "'".$user_ex[0]."'";
    								if($user_ex[0] == "pi"){
    									echo '<tr><td>'.$user_ex[0].'</td><td><button class="btn btn-raised btn-primary" onClick="changePwd('.$u1.');">Change Password</button>&nbsp;&nbsp;You cannot delete the user \'pi\', apache is running as this user.</td></tr>';
    								} else {
    									if($user_ex[0] == $_SESSION['username']){
    										echo '<tr><td>'.$user_ex[0].'</td><td><button class="btn btn-raised btn-primary" onClick="changePwd('.$u1.');">Change Password</button>&nbsp;&nbsp;You cannot delete the user you are logged in as.</td></tr>';
    									} else {
    										echo '<tr><td>'.$user_ex[0].'</td><td><button class="btn btn-raised btn-primary" onClick="changePwd('.$u1.');">Change Password</button>&nbsp;&nbsp;<button class="btn btn-raised btn-warning" onClick="delUser('.$u1.');">Delete User</button></td></tr>';
    									}
    								}
